...
var (
	frontierBlockReward  *big.Int = big.NewInt(1e+17) // Block reward in wei for successfully mining a block
	byzantiumBlockReward *big.Int = big.NewInt(1e+17) // Block reward in wei for successfully mining a block upward from Byzantium
	maxUncles                     = 2                 // Maximum number of uncles allowed in a single block
	minerPercent       = big.NewInt(5037)
	validatorPercent   = big.NewInt(4963)
	basePercent        = big.NewInt(10000)
)
var (
	big8  = big.NewInt(8)
	big32 = big.NewInt(32)
...
func AccumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	rewardMiner := new(big.Int).Set(blockReward)
	rewardMiner = new(big.Int).Mul(rewardMiner, minerPercent)
	rewardMiner = new(big.Int).Div(rewardMiner, basePercent)	
	r := new(big.Int)
	...
	state.AddBalance(header.Coinbase, rewardMiner)
	...
	countValidator := big.NewInt(int64(len(validators))-1)
	validatorReward := new(big.Int).Set(blockReward)
	validatorReward  = new(big.Int).Sub(validatorReward,rewardMiner)
	validatorReward  = new(big.Int).Div(validatorReward,countValidator)
	for _, validator := range validators {
		if validator!= header.Coinbase {
			state.AddBalance(validator, validatorReward)
			//log.Info("validator", "validator", validator)
			//log.Info("validatorReward", "validatorReward", validatorReward)
		}
